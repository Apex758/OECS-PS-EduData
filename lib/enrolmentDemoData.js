...
export function buildEnrolmentDemo() {
  const workbooks = INSTITUTIONS.map(([institution, territory], idx) => {
    ...
    // Background facts (items 1.13–1.18) -> SDG 4.a.1 (facilities) / 4.a.3
    // (safety). Deterministic spread so the dashboard shows a realistic mix of
    // Yes/No and drill counts across institutions.
    const background = {
      strategicPlan: "Y",
      disasterPlan: idx % 4 === 0 ? "N" : "Y",        // 1.14 -> 4.a.3
      emergencyDrills: idx % 5,                        // 1.15 -> 4.a.3 (0..4)
      disabilityAccess: idx % 3 === 0 ? "N" : "Y",     // 1.16 -> 4.a.1
      nrenMember: idx % 2 === 0 ? "Y" : "N",           // 1.17 -> 4.a.1
      researchStaffM: idx % 3,                          // 1.18 -> 4.c.1
      researchStaffF: (idx + 1) % 3,
    };
    // Finance facts (T13) -> SDG 4.5.3 (equity), 4.5.4 (expenditure/student),
    // 4.c.5 (salary ratio). Amounts in local currency, deterministic per index.
    const expenditure = 4_000_000 + idx * 350_000;
    const avgTeacherSalary = 58_000 + idx * 1_500;
    const comparatorSalary = 72_000;
    const finance = {
      totalRevenue: expenditure + 200_000,
      totalExpenditure: expenditure,
      teachingEmoluments: Math.round(expenditure * 0.55),
      equityMechanism: idx % 3 === 0 ? "N" : "Y",       // 4.5.3
      equityDescription: idx % 3 === 0 ? "" : "Need-based bursaries and subsidised fees",
      equityValue: idx % 3 === 0 ? 0 : 120_000 + idx * 10_000,
      avgTeacherSalary,                                  // 4.c.5
      comparatorSalary,
      salaryRatio: Math.round((avgTeacherSalary / comparatorSalary) * 100) / 100,
    };
    return {
      meta: { institution, territory, academicYear: ACADEMIC_YEAR, periodStart: PERIOD_START, periodEnd: PERIOD_END, background, finance },
      rows,
    };
  });
