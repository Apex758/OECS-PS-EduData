...
// ---------------------------------------------------------------------
export async function ingestEnrolment({ meta, rows }) {
  return unwrap(
    await svc().rpc("ingest_enrolment", { p_meta: meta || {}, p_rows: rows || [] })
  );
}

// DB snake_case -> camelCase used by the SDG math (lib/sdgEnrolment.js).
const ENROL_SELECT =
  ...
  "other_nat_m, other_nat_f, oda_scholarship, created_at";
...
// Wipe all enrolment rows. service_role only (used by /api/clear).
export async function clearEnrolment() {
  unwrap(await svc().from("enrolment").delete().gte("id", 0));
}
