...
// Exe record-push: already-anonymized, SAFE-ONLY staff rows (no PII) pushed by
// a standalone to populate the stats/SDG dashboards -- the data counterpart to
// the validation-layer token push. Rows arrive pre-shaped and pre-normalized
// (the exe ran the same pipeline locally), each carrying ruli + identity_hash +
// salt + the safe columns, but NO `sensitive` key. ingest_staff stores an empty
// mapping (coalesce sensitive -> '{}'), so nothing written here can re-identify
// anyone. rows: [{ ruli, institution, territory, ...safe, identity_hash, salt,
// metadata }]. Returns { ok, inserted, skipped, rejected, institutions }.
export async function ingestStaffRows({ rows, rejected = [] }) {
  const p_rejected = (rejected || []).map((r) => ({ data: r.data ?? {}, errors: r.errors ?? [] }));
  return unwrap(await svc().rpc("ingest_staff", { p_rows: rows || [], p_rejected }));
}
...
function oecsVisibleRow(row, approvedIds, { legacyVisible = true } = {}) {
  if (row.submission_id == null) return legacyVisible;
  return approvedIds.has(row.submission_id);
}

async function approvedSubmissionIds() {
  const rows =
    unwrap(await svc().from("submissions").select("id").eq("status", "approved")) || [];
  return new Set(rows.map((r) => r.id));
}

async function applyOecsVisibility(rows, approvedOnly) {
  if (!approvedOnly) return rows;
  const approved = await approvedSubmissionIds();
  return rows.filter((r) => oecsVisibleRow(r, approved, { legacyVisible: false }));
}

export async function readStaffRecords(filter = {}) {
  const needApproval = !!filter.approvedOnly;
  let q = svc().from("staff").select(
    needApproval ? `${STAFF_SELECT}, submission_id` : STAFF_SELECT
  );
  if (filter.countryId) q = q.eq("country_id", filter.countryId);
  if (filter.schoolId) q = q.eq("school_id", filter.schoolId);
  let rows =
    unwrap(await q.order("created_at", { ascending: true }).order("ruli", { ascending: true })) || [];
  if (needApproval) rows = await applyOecsVisibility(rows, true);
  return rows.map((r) => {
    const staff = {};
...
export async function readStaffRejected(filter = {}) {
  let q = svc().from("staff_rejected").select("data, errors, created_at, submission_id");
  if (filter.countryId || filter.schoolId) {
    q = q.not("submission_id", "is", null);
  }
  const rows = unwrap(await q.order("created_at", { ascending: true })) || [];
  let out = rows.map((r) => ({ data: r.data || {}, errors: r.errors || [], submissionId: r.submission_id }));
  if (filter.countryId || filter.schoolId) {
    const subIds = [...new Set(out.map((r) => r.submissionId).filter(Boolean))];
    if (!subIds.length) return [];
    let sq = svc().from("submissions").select("id, status").in("id", subIds);
    if (filter.countryId) sq = sq.eq("country_id", filter.countryId);
    if (filter.schoolId) sq = sq.eq("school_id", filter.schoolId);
    if (filter.approvedOnly) sq = sq.eq("status", "approved");
    const allowed = new Set((unwrap(await sq) || []).map((s) => s.id));
    out = out.filter((r) => allowed.has(r.submissionId));
  } else if (filter.approvedOnly) {
    const subIds = [...new Set(out.map((r) => r.submissionId).filter(Boolean))];
    if (subIds.length) {
      const approved = new Set(
        (unwrap(await svc().from("submissions").select("id").in("id", subIds).eq("status", "approved")) || [])
          .map((s) => s.id)
      );
      out = out.filter((r) => r.submissionId && approved.has(r.submissionId));
    } else {
      out = [];
    }
  }
  return out.map(({ submissionId, ...r }) => r);
}
...
export async function readEnrolment(filter = {}) {
  const needApproval = !!filter.approvedOnly;
  let q = svc().from("enrolment").select(`${ENROL_SELECT}, submission_id`);
  if (filter.countryId) q = q.eq("country_id", filter.countryId);
  if (filter.schoolId) q = q.eq("school_id", filter.schoolId);
  let rows =
    unwrap(
...
export async function readEnrolmentRejected(filter = {}) {
  let q = svc()
    .from("enrolment_rejected")
    .select("institution, academic_year, data, errors, created_at, submission_id");
  if (filter.countryId || filter.schoolId) {
    q = q.not("submission_id", "is", null);
  }
  const rows = unwrap(await q.order("created_at", { ascending: true })) || [];
...
export async function clearApprovalLayer() {
  const sb = svc();
  unwrap(await sb.from("approvals").delete().gte("id", 0));
  unwrap(await sb.from("aggregations").delete().gte("id", 0));
  unwrap(await sb.from("submissions").delete().gte("id", 0));
}
...
// Delete tokens by their exact token string (PK). Cascade for when an exe
// deletes a saved mapping run locally: remove its tokens here too. After the
// delete, re-evaluate each touched salt -- if its cross-RULI collision is gone
// (<=1 distinct RULI left), drop the PENDING dup candidate for that salt. A
// dup still backed by >1 institution's RULIs is kept; decided dups (approved/
// denied) are left untouched as an audit record. tokens: string[].
export async function valDeleteTokens(tokens) {
  const list = [...new Set((tokens || []).filter((t) => typeof t === "string" && t))];
  if (!list.length) return { deleted: 0, dupsCleared: 0 };
  const sb = svc();

  // Salts the doomed tokens belong to -- their dups need re-checking after.
  const touched = unwrap(await sb.from("validation_tokens").select("salt").in("token", list)) || [];
  const salts = [...new Set(touched.map((r) => r.salt))];

  const removed = unwrap(await sb.from("validation_tokens").delete().in("token", list).select("token")) || [];

  let dupsCleared = 0;
  for (const salt of salts) {
    const rows = unwrap(await sb.from("validation_tokens").select("ruli").eq("salt", salt)) || [];
    if (new Set(rows.map((r) => r.ruli)).size > 1) continue; // collision still real -> keep dup
    const del =
      unwrap(await sb.from("validation_dups").delete().eq("salt", salt).eq("status", "pending").select("id")) || [];
    dupsCleared += del.length;
  }
  return { deleted: removed.length, dupsCleared };
}

// Wipe the whole validation layer back to empty: stored salts/tokens, dup
// candidates, and the activity log. Registered exe keys (ruli_mapper_keys) are
// KEPT so the standalones can still authenticate after a reset. service_role.
export async function clearValidation() {
  const sb = svc();
  const tok = unwrap(await sb.from("validation_tokens").delete().neq("token", "").select("token")) || [];
  unwrap(await sb.from("validation_dups").delete().gte("id", 0).select("id"));
  unwrap(await sb.from("validation_events").delete().gte("id", 0).select("id"));
  return { tokensCleared: tok.length };
}
...
// Purge audit events. Pass kinds to drop only those; omit to wipe all.
export async function valDeleteEvents({ kinds = null } = {}) {
  let q = svc().from("validation_events").delete();
  q = kinds && kinds.length ? q.in("kind", kinds) : q.gte("id", 0);
  const rows = unwrap(await q.select("id")) || [];
...
// Unregister exe key(s). Pass an id to drop one, or omit it to wipe all
// (demo reset). Returns how many rows were removed.
export async function deleteRuliKey({ id = null } = {}) {
  let q = svc().from("ruli_mapper_keys").delete();
  q = id ? q.eq("id", id) : q.gte("id", 0);
  const rows = unwrap(await q.select("id")) || [];
  return { ok: true, deleted: rows.length };
}
...
// =====================================================================
// APPROVAL LAYER  --  submissions, aggregations, minister queue
// =====================================================================

export async function submitStaffSubmission({ userId, rows, rejected, aggregations }) {
  const p_rejected = (rejected || []).map((r) => ({ data: r.data ?? {}, errors: r.errors ?? [] }));
  return unwrap(
    await svc().rpc("submit_staff_submission", {
      p_rows: rows || [],
      p_rejected,
      p_user_id: userId,
...
export async function submitEnrolmentSubmission({ userId, meta, rows, rejected, aggregations }) {
  const p_rejected = (rejected || []).map((r) => ({ data: r.data ?? {}, errors: r.errors ?? [] }));
  return unwrap(
    await svc().rpc("submit_enrolment_submission", {
      p_meta: meta || {},
      p_rows: rows || [],
      p_rejected,
      p_user_id: userId,
...
export async function approveSubmissionL2({ submissionId, ministerId }) {
  return unwrap(
    await svc().rpc("approve_submission_l2", {
...
export async function rejectSubmission({ submissionId, ministerId, reason }) {
  return unwrap(
    await svc().rpc("reject_submission", {
      p_submission_id: submissionId,
      p_minister_id: ministerId,
      p_reason: reason || "",
    })
...
export async function listPendingSubmissions(countryId) {
  return unwrap(await svc().rpc("list_pending_submissions", { p_country_id: countryId })) || [];
}

export async function listSubmissionsForUser(userId) {
  return unwrap(await svc().rpc("list_submissions_for_user", { p_user_id: userId })) || [];
}

export async function listAggregationsOecs() {
  const rows =
    unwrap(
      await svc()
        .from("aggregations")
        .select(
          "id, sdg, numerator, denominator, result, metadata, country_id, school_id, submission_id, countries(iso_code, name), schools(code, name), submissions!inner(status)"
        )
  ...
  return rows.map((r) => ({
    ...
    campus: r.schools?.name ?? null,
    campusCode: r.schools?.code ?? null,
...
export async function listAggregationsScoped({ role, userId, countryId, schoolIds }) {
  if (role === "admin") return listAggregationsOecs();

  let q = svc()
    .from("aggregations")
    .select(
      "id, sdg, numerator, denominator, result, metadata, country_id, school_id, submission_id, countries(iso_code, name), schools(code, name), submissions(status), approvals(l1, l2)"
    )
    .order("sdg")
    .order("id");
  ...
  } else if (role === "teacher" && schoolIds?.length) {
  ...
  return rows
    .filter((r) => role !== "admin" || r.submissions?.status === "approved")
    .map((r) => ({
      id: r.id,
      sdg: r.sdg,
      country: r.countries?.iso_code ?? null,
      ...
      metadata: r.metadata,
      submissionId: r.submission_id,
      status: r.submissions?.status ?? null,
      l1: r.approvals?.l1 ?? null,
      l2: r.approvals?.l2 ?? null,
    }));
}

export async function getSubmissionAggregations(submissionId) {
  const rows =
    unwrap(
      await svc()
        .from("aggregations")
        .select("id, sdg, numerator, denominator, result, metadata, approvals(l1, l2, l2_at)")
...
export async function listApprovalConfig() {
  return (
    unwrap(
      await svc()
        .from("approval_config")
        .select("country_id, approval_required, countries(iso_code, name)")
        .order("countries(iso_code)")
    ) || []
  ).map((r) => ({
    countryId: r.country_id,
    iso: r.countries?.iso_code ?? null,
    name: r.countries?.name ?? null,
...
export async function getUserSchoolIds(userId) {
  const rows =
    unwrap(
      await svc().from("user_schools").select("school_id").eq("user_id", userId)
    ) || [];
  return rows.map((r) => r.school_id);
}

export async function getCountryIdByIso(iso) {
  if (!iso) return null;
  const row = unwrap(
    await svc().from("countries").select("id, iso_code, name").eq("iso_code", String(iso).toUpperCase()).maybeSingle()
  );
  return row?.id ?? null;
}
...
export async function listSchools({ countryIso } = {}) {
  let q = svc()
    .from("schools")
    .select("id, code, name, country_id, countries(iso_code, name)")
    .order("code");
  if (countryIso) {
    const cid = await getCountryIdByIso(countryIso);
    if (cid) q = q.eq("country_id", cid);
  }
  return (unwrap(await q) || []).map((s) => ({
    id: s.id,
    code: s.code,
    name: s.name,
...
export async function getUserPrimarySchool(userId) {
  const rows =
    unwrap(
      await svc()
        .from("user_schools")
        .select("school_id, schools(id, code, name, country_id, countries(iso_code, name))")
        .eq("user_id", userId)
        .limit(1)
    ) || [];
  const link = rows[0];
  if (!link?.schools) return null;
  const s = link.schools;
  return {
    schoolId: s.id,
    schoolCode: s.code,
    schoolName: s.name,
...
export async function resolveDbScopeFromQuery({ country, school } = {}) {
  const scope = {};
  if (country) {
    scope.countryId = await getCountryIdByIso(country);
    if (!scope.countryId) return { error: `unknown country '${country}'` };
  }
  if (school) {
    const sc = await getSchoolByCode(school);
    if (!sc) return { error: `unknown school '${school}'` };
    scope.schoolId = sc.id;
    if (scope.countryId && scope.countryId !== sc.country_id) {
      return { error: `school '${school}' is not in country '${country}'` };
    }
    scope.countryId = scope.countryId ?? sc.country_id;
  }
  return { scope };
}

export async function getApprovalRequiredForCountry(countryId) {
  const rows =
    unwrap(
      await svc()
        .from("approval_config")
  ...
  return rows?.approval_required ?? false;
}
