...
function oecsVisibleRow(row) {
  if (row.submission_id == null) return true;
  return row.submissions?.status === "approved";
}

export async function readStaffRecords(filter = {}) {
  const needApproval = !!filter.approvedOnly;
  let q = svc().from("staff").select(
    needApproval ? `${STAFF_SELECT}, submission_id, submissions(status)` : STAFF_SELECT
  );
  ...
  if (needApproval) rows = rows.filter(oecsVisibleRow);
  return rows.map((r) => {
    const staff = {};
...
export async function readEnrolment(filter = {}) {
  const needApproval = !!filter.approvedOnly;
  const select = needApproval ? `${ENROL_SELECT}, submission_id, submissions(status)` : ENROL_SELECT;
  let q = svc().from("enrolment").select(select);
  if (filter.countryId) q = q.eq("country_id", filter.countryId);
  if (filter.schoolId) q = q.eq("school_id", filter.schoolId);
  let rows =
    unwrap(
